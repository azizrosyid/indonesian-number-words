...
export class Terbilang {
   ...
   */
  static Convert(convertNumber: Decimal | number, firstTry = true): string {
    // Input validation
    if (
      typeof convertNumber !== "number" &&
      !(convertNumber instanceof Decimal)
    ) {
    ...
    if (!fractionalPart.isZero()) {
      ...
      const fractionalWords = Array.from(fractionalString).map(
        (digit) => this.words[parseInt(digit)]
      );
      result +=
        " koma " +
